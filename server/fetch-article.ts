...
/**
 * Article fetcher — the server half of importing a link.
 *
 ...
 * the app. So Something's own server does the fetch: the local dev server
 * (`vite-plugin-fetch.ts`) when running from source, and the `/api/fetch`
 * function (`api/fetch.ts`) on the public deployment. There, the address of the
 * page being imported passes through Something's server — the page's text is
 * parsed in the reader's browser and never stored or logged here.
 *
 * An endpoint that fetches user-supplied URLs is an SSRF primitive, so it is
 * treated as hostile input end to end:
 *
 *   - only same-origin callers, enforced with a header a cross-origin page
 *     cannot set without a preflight this server never answers. That stops
 *     other websites from using a visitor's browser as a proxy; it does not
 *     stop a script that sets the header itself, which is why the public
 *     endpoint is also rate-limited per address;
    ...
    const req = transport.request(
      target,
      {
        ...
        headers: {
          ...
          "user-agent": "something-reader (+https://github.com/caio-creator/something-reader)",
        },
        timeout: TIMEOUT_MS,
      },
      (res: IncomingMessage) => {
 ...
 */
const isSameOrigin = (header: (name: string) => string | undefined): boolean => {
  if (header("x-something-reader") !== "1") return false;
...
export type FetchRequest = {
  ...
  /** Lower-case header lookup; undefined when absent. */
  header: (name: string) => string | undefined;
  /** The `url` query parameter, or null. */
  target: string | null;
};

export type FetchReply = { status: number; body: { url: string; html: string } | { error: string } };

/**
 * The whole endpoint, independent of what serves it. The dev server and the
 * public function both answer with exactly this, so the rules cannot drift
 * between where they are tested and where they run.
 */
export const handleFetch = async (request: FetchRequest, deps: Deps = {}): Promise<FetchReply> => {
  if (request.method !== "GET") return { status: 405, body: { error: "Use GET." } };
  if (!isSameOrigin(request.header)) return { status: 403, body: { error: "That request did not come from the app." } };
  if (!request.target) return { status: 400, body: { error: "Give me a link." } };

  let parsed: URL;
  try {
    parsed = new URL(request.target);
  } catch {
    return { status: 400, body: { error: "That does not look like a link." } };
  }

  try {
    return { status: 200, body: await fetchArticle(parsed.toString(), deps) };
  } catch (err) {
    return { status: 400, body: { error: err instanceof Error ? err.message : "Could not reach that link." } };
  }
};

/** Headers every answer carries: never cached, and never shared across origins. */
